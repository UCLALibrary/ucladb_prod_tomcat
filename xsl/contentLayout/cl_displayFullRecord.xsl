...
<xsl:variable name="Config" select="document('./configs/displayfullcfg.xml')"/>
...
<xsl:template name="buildFullRecordForm">
			...
			<div class="recordContent">
				...
				<!-- ## Action Box ## -->
                                ...
                                <xsl:variable name="moreActions">
                                        <li><span class="recordPointer"></span><label>Detailed Record</label></li>
                                </xsl:variable>

				<xsl:call-template name="buildActionBox">
					<xsl:with-param name="pageRecordType" select="'actionBox.fullRecordView.link'"/>
